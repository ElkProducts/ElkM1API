...
namespace Elk {
	...
	public:
		...
		ELKM1API int getTemperature(Elk::M1API::TemperatureDevice type, int device);
		ELKM1API std::vector<int> getTemperatures(Elk::M1API::TemperatureDevice type);
		ELKM1API Elk::M1API::KeypadFkeyStatus getKeypadFkeyStatus(int keypad);
		ELKM1API Elk::M1API::LogEntry getLogData(int index);
		ELKM1API std::vector<int> getPLCStatus(int bank);
		ELKM1API Elk::M1API::RTCData getRTCData();
		ELKM1API Elk::M1API::RTCData setRTCData(Elk::M1API::RTCData newData);
		...
		ELKM1API Elk::M1API::SystemTroubleStatus getSystemTroubleStatus();
		ELKM1API Elk::M1API::ThermostatData getThermostatData(int index);
		ELKM1API Elk::M1API::ThermostatData setThermostatData(int index, int value, int element);
		...
		ELKM1API Elk::M1API::UserCodeAccess getUserCodeAccess(std::string userCode);
		ELKM1API Elk::M1API::UserCodeSuccess requestChangeUserCode(int user, std::string authCode, std::string newUserCode, uint8_t areaMask);
		ELKM1API void activateTask(int taskNumber);
		ELKM1API void armDisarm(int partition, Elk::M1API::ArmMode mode, std::string userCode);
		ELKM1API void disableControlOutput(int output);
		ELKM1API void displayLCDText(int area, Elk::M1API::clearMethod clear, bool beepKeypad, uint16_t displayTime, std::string text);
		ELKM1API void displayLCDText(int area, Elk::M1API::clearMethod clear, bool beepKeypad, uint16_t displayTime, std::string lineOne, std::string lineTwo);
